...
int tdx_mcall_tdreport(u64 data, u64 reportdata)
{
	...
	/*
	 * Use protected guest TDX check to ensure this API is only
	 ...
	 * tdreport data to the TDX module to generate the TDREPORT.
	 * TDREPORT data contains measurements/configuration data
	 * of the TD guest. More info about ABI can be found in TDX
...
int tdx_mcall_rtmr_extend(u64 data, u64 rtmr)
{
	...
	ret = __trace_tdx_module_call(TDRMTR_EXTEND, data, rtmr, 0, 0, NULL);
...
void __init tdx_early_init(void)
{
	...
	if (!cpuid_has_tdx_guest() && !tdx_guest_forced)
		return;
